...
# DEFCONFIG
DEFCONFIG1="gki_defconfig"
DEFCONFIG2="vendor/pineapple_GKI.config"
DEFCONFIG3="vendor/oplus/pineapple_GKI.config"

# AnyKernel3
AnyKernel="https://github.com/bheatleyyy/AnyKernel3.git"
AnyKernelbranch="oscar"
...
export IMG="$PWD"/out/arch/arm64/boot/Image
export dtbo="$PWD"/out/arch/arm64/boot/dtbo.img
        ...
        if [ -f "$IMG" ]; then
                ...
                cp -r "$dtbo" zip/
                cd zip
                export ZIP="LineageOS-avalon-$(date '+%Y%m%d-%H%M').zip"
